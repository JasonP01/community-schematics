...
class CustomBuffer {
	...
	skip(amount: number = 1): this {
		this.checkCanRead(amount)
	...
	skipShort(amount: number = 1): this {
		this.checkCanRead(amount * 2)

		this.#offset += amount * 2

		return this
	}

	skipInt(amount: number = 1): this {
		this.checkCanRead(amount * 4)

		this.#offset += amount * 4

		return this
	}

	skipLong(amount: number = 1): this {
		this.checkCanRead(amount * 8)

		this.#offset += amount * 8

		return this
	}

	skipFloat(amount: number = 1): this {
		this.checkCanRead(amount * 4)
	...
	skipDouble(amount: number = 1): this {
		this.checkCanRead(amount * 8)
	...
	skipUByte(amount: number = 1): this {
		this.checkCanRead(amount)
	...
	skipUShort(amount: number = 1): this {
		this.checkCanRead(amount * 2)
	...
	skipBoolean(amount: number = 1): this {
		return this.skip(amount)
	}

	skipUTF(amount: number = 1): this {
	...
	skipString(amount: number = 1): this {
		for (let i = 0; i < amount; i++) {
	...
	write(value: Byte): void {
		if (!value.signed) throw new Error('Cannot write unsigned byte to signed')
	...
	writeShort(value: Short): void {
		if (!value.signed) throw new Error('Cannot write unsigned short to signed')
	...
	writeInt(value: Int): void {
		if (!value.signed) throw new Error('Cannot write unsigned int to signed')
	...
	writeLong(value: Long): void {
		if (!value.signed) throw new Error('Cannot write unsigned long to signed')
	...
	writeFloat(value: Float): void {
		if (!value.signed) throw new Error('Cannot write unsigned float to signed')
	...
	writeDouble(value: Double): void {
		if (!value.signed) throw new Error('Cannot write unsigned double to signed')
	...
	writeUByte(value: Byte): void {
		if (value.signed) throw new Error('Cannot write signed byte to unsigned')
	...
	writeUShort(value: Short): void {
		if (value.signed) throw new Error('Cannot write signed short to unsigned')
	...
	set offset(value: number) {
		if (this.#offset + value > this.capacity) return
		this.#offset = value
	}
